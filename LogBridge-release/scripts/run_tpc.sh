...
DATA="$REPO/datasets/tpc"
KG="$DATA/kg_tpc_v3.json"
...
case "$mode" in
  train)
    cd "$REPO" && $PYTHON src/training/train.py \
      ...
      2>&1 | tee "$REPO/results/tpc/train.log"
    ;;
  eval_single_window)
    cd "$REPO" && $PYTHON src/training/eval_single_window.py --data-root "$DATA" \
      2>&1 | tee "$REPO/results/tpc/eval_single_window.log"
    ;;
  lt_only)
    cd "$REPO" && $PYTHON src/training/train_lt_only.py \
      ...
      2>&1 | tee "$REPO/results/tpc/lt_only.log"
    ;;
